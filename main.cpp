...
int main()
{
	...
	while (window.isOpen())
	{
		...
		if ((rightBat.getPosition().x <= midRightX && midRightX <= rightBat.getPosition().x + batWidth) && (rightBat.getPosition().y <= midRightY && midRightY <= leftBat.getPosition().y + batHeight))
		{
			ball_speedx = -ball_speedx;
		}
